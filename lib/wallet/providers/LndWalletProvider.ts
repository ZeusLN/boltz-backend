...
import { AddressType } from '../../proto/lnd/rpc';
...
class LndWalletProvider implements WalletProviderInterface {
  ...
  public getBalance = (): Promise<WalletBalance> => {
    return this.lndClient.getBalance();
  };
  ...
  private handleLndTransaction = async (
    ...
    for (const transaction of transactions) {
      if (transaction.txHash === transactionId) {
        fee = fromProtoInt(transaction.totalFees);
        break;
      }
    }
    ...
    return {
      ...
      transaction: Transaction.fromRaw(hexToBytes(rawTransaction.hex)),
    };
  };
